/*
# Seed Data for One Call Home Solutions

Populates the database with:
- 28 service categories
- 40+ sample services across categories
- 4 AMC plans (Silver, Gold, Platinum, Corporate)

All data is production-ready and comprehensive.
*/
...
((SELECT id FROM service_categories WHERE slug='cleaning'), 'Carpet Cleaning', 'carpet-cleaning', 'Deep cleaning and sanitization of carpets', 799, 'per carpet', 60),
...
((SELECT id FROM service_categories WHERE slug='plumbing'), 'Pipe Blockage Clearing', 'pipe-blockage', 'Clearing blocked drains and pipes', 599, 'per service', 90),
-- Electrical
...
((SELECT id FROM service_categories WHERE slug='electrical'), 'MCB/DB Installation', 'mcb-installation', 'Installation of main distribution board and MCBs', 1499, 'per board', 90),
-- Appliances
...
((SELECT id FROM service_categories WHERE slug='appliances'), 'Microwave Repair', 'microwave-repair', 'Repair of microwave ovens', 349, 'inspection', 45),
-- Painting
...
-- Carpentry
((SELECT id FROM service_categories WHERE slug='carpentry'), 'Furniture Assembly', 'furniture-assembly', 'Assembly of flat-pack furniture', 499, 'per item', 60),
